...
namespace bd {
  OutputService::OutputService(QSharedPointer<OutputManager::Wlr::MetaHead> output, QObject* parent) : QObject(parent), m_output(output) {
    QString objectPath = QString("/org/buddiesofbudgie/Services/Displays/Outputs/%1").arg(output->getIdentifier());
  ...
  QStringList OutputService::GetAvailableModes() {
    ...
    for (const auto& mode : m_output->getModes()) {
      modePaths << QString("/org/buddiesofbudgie/Services/Displays/Outputs/%1/Modes/%2").arg(m_output->getIdentifier()).arg(mode->getId());
    }
    return modePaths;
  }
  ...
  QString OutputService::GetCurrentMode() {
    ...
    if (mode) { return QString("/org/buddiesofbudgie/Services/Displays/Outputs/%1/Modes/%2").arg(m_output->getIdentifier()).arg(mode->getId()); }
    return QString();
  }
  ...
  QString OutputService::HorizontalAnchor() const {
    return bd::BatchSystem::ConfigurationHorizontalAnchor::toString(m_output->getHorizontalAnchor());
  }
  ...
  QString OutputService::VerticalAnchor() const {
    return bd::BatchSystem::ConfigurationVerticalAnchor::toString(m_output->getVerticalAnchor());
  }
