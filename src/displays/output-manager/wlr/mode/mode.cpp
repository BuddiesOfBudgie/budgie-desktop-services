...
namespace bd::OutputManager::Wlr {
  ...
  void Mode::zwlr_output_mode_v1_size(int32_t width, int32_t height) {
    emit propertyChanged(MetaModeProperty::Size, QVariant {QSize(width, height)});
  }

  void Mode::zwlr_output_mode_v1_refresh(int32_t refresh) {
    auto val = QVariant::fromValue(refresh);
    emit propertyChanged(MetaModeProperty::Refresh, val);
  }

  void Mode::zwlr_output_mode_v1_preferred() {
    emit propertyChanged(MetaModeProperty::Preferred, QVariant::fromValue(true));
  }
