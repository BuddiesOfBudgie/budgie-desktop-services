...
namespace bd {
  ...
  void DisplayConfig::apply() {
    ...
    auto& orchestrator = OutputManager::WaylandOrchestrator::instance();
    ...
    auto& batchSystem = bd::BatchSystem::OutputBatchSystem::instance();
    batchSystem.reset();

    // Connect to batch system completion signals
    connect(
        &batchSystem, &bd::BatchSystem::OutputBatchSystem::configurationApplied, this,
        [this, &batchSystem](bool success) {
          ...
          disconnect(&batchSystem, &bd::BatchSystem::OutputBatchSystem::configurationApplied, this, nullptr);
        },
    ...
    for (const auto& serial : group->getOutputIdentifiers()) {
      ...
      if (config->getDisabled()) {
        ...
        auto offAction = bd::BatchSystem::ConfigurationAction::explicitOff(serial);
        batchSystem.addAction(offAction);
        qDebug() << "  - Disable output";
      } else {
        // Create action to enable this output
        auto onAction = bd::BatchSystem::ConfigurationAction::explicitOn(serial);
        batchSystem.addAction(onAction);

        // Set mode (dimensions and refresh)
        auto modeAction = bd::BatchSystem::ConfigurationAction::mode(serial, QSize(config->getWidth(), config->getHeight()), config->getRefresh());
        ...
        if (!relativeOutput.isEmpty()) {
          ...
          auto anchorAction     = bd::BatchSystem::ConfigurationAction::setPositionAnchor(serial, relativeOutput, horizontalAnchor, verticalAnchor);
          ...
          if (!head.isNull()) {
            ...
            head->setHorizontalAnchoring(bd::BatchSystem::ConfigurationHorizontalAnchor::NoHorizontalAnchor);
            head->setVerticalAnchoring(bd::BatchSystem::ConfigurationVerticalAnchor::NoVerticalAnchor);
          }
          qDebug() << "  - No anchoring set";
        }

        // Set scale
        auto scaleAction = bd::BatchSystem::ConfigurationAction::scale(serial, config->getScale());
        batchSystem.addAction(scaleAction);

        // Set transform (rotation)
        auto transformAction = bd::BatchSystem::ConfigurationAction::transform(serial, static_cast<quint8>(config->getRotation()));
        batchSystem.addAction(transformAction);

        // Set adaptive sync
        auto adaptiveSyncAction = bd::BatchSystem::ConfigurationAction::adaptiveSync(serial, config->getAdaptiveSync() ? 1 : 0);
  ...
  void DisplayConfig::debugOutput() {
    for (const auto& group : this->m_groups) {
      ...
      for (auto config : group->getConfigs()) {
        ...
        qDebug() << "    Horizontal Anchor: " << bd::BatchSystem::ConfigurationHorizontalAnchor::toString(config->getHorizontalAnchor());
        qDebug() << "    Vertical Anchor: " << bd::BatchSystem::ConfigurationVerticalAnchor::toString(config->getVerticalAnchor());
        qDebug() << "    Scale: " << config->getScale();
        qDebug() << "    Rotation: " << config->getRotation();
        qDebug() << "    Adaptive Sync: " << config->getAdaptiveSync();
  ...
  DisplayGroup::DisplayGroup(const toml::value& v, QObject* parent) : QObject(parent) {
    ...
    for (const toml::value& output : outputs) {
      ...
      dgo->setHorizontalAnchor(bd::BatchSystem::ConfigurationHorizontalAnchor::fromString(toml::find_or<std::string>(output, "horizontal_anchor", "none")));
      dgo->setVerticalAnchor(bd::BatchSystem::ConfigurationVerticalAnchor::fromString(toml::find_or<std::string>(output, "vertical_anchor", "none")));
  ...
  DisplayGroupOutputConfig::DisplayGroupOutputConfig(QObject* parent)
      : QObject(parent),
        ...
        m_refresh(0),
        m_relative_output(""),
        m_horizontal_anchor(bd::BatchSystem::ConfigurationHorizontalAnchor::Type::NoHorizontalAnchor),
        m_vertical_anchor(bd::BatchSystem::ConfigurationVerticalAnchor::Type::NoVerticalAnchor),
        m_scale(1.0),
        m_rotation(0),
  ...
  bd::BatchSystem::ConfigurationHorizontalAnchor::Type DisplayGroupOutputConfig::getHorizontalAnchor() const {
    return this->m_horizontal_anchor;
  }

  bd::BatchSystem::ConfigurationVerticalAnchor::Type DisplayGroupOutputConfig::getVerticalAnchor() const {
  ...
  void DisplayGroupOutputConfig::setHorizontalAnchor(bd::BatchSystem::ConfigurationHorizontalAnchor::Type horizontalAnchor) {
    this->m_horizontal_anchor = horizontalAnchor;
  }

  void DisplayGroupOutputConfig::setVerticalAnchor(bd::BatchSystem::ConfigurationVerticalAnchor::Type verticalAnchor) {
  ...
  toml::ordered_value DisplayGroupOutputConfig::toToml() {
    ...
    config_table["horizontal_anchor"] = bd::BatchSystem::ConfigurationHorizontalAnchor::toStringStd(this->m_horizontal_anchor);
    config_table["vertical_anchor"]   = bd::BatchSystem::ConfigurationVerticalAnchor::toStringStd(this->m_vertical_anchor);
